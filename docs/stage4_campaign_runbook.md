...
run, select protocol `v3`; v1 and v2 are retained only to inspect their
immutable evidence. V2 failed its promotion gate and must never be rerun.
...
Do not dispatch calibration until the v3 engineering-smoke workflow is green.
Do not dispatch validation or robustness until the v3 calibration verifier is
green. The workflow defaults to v3 so a normal dispatch cannot accidentally
reuse an earlier protocol or seed namespace.

V3 scores all inferential metrics only over accepted, declared inferential
records. Strong-complete-graph recovery requires exact recovery of every true
pairwise edge in every inferential strong-overlap cell; expected rare-group
refusals are scored separately.
