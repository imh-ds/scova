...
def boundary_diagnostic(
    ...
    candidate: CFSupportProfile | None,
    calibration: dict[str, Any],
) -> dict[str, Any]:
    """Estimate a density boundary after candidate selection, never adopt it."""
    base = {
        "artifact_type": "scova-cf-boundary-diagnostic",
        "schema_version": 1,
        "program_type": "qualification",
        "verification_lane": "boundary",
    ...
    for index, cell in enumerate(protocol.retained_cells):
        records = [
            r for r in calibration["records"] if int(r["cell_index"]) == index and not r["refused"]
        ]
    ...
    result: dict[str, Any] = {
        **base,
        "design_adequacy": {str(k): len(v) for k, v in design.items()},
        "realized_fit_adequate": realized_adequate,
    }
    ...
    target = math.log(
        ...
        / (1 - float(protocol.metrics["minimum_strong_cell_pass_fraction"]))
    )
    points = {
        stratum: float((target - coefficient[1 + (stratum == 3)]) / coefficient[0])
    ...
    for _ in range(1000):
        ...
        if len(set(y[sample])) < 2:
            continue
        fitted = (
            LogisticRegression(penalty=None, fit_intercept=False, max_iter=5000)
    ...
    intervals = {
        stratum: [float(np.quantile(draws[stratum], q)) for q in (0.025, 0.975)]
        for stratum in strata
        if draws[stratum]
    }
    informative = len(intervals) == 2 and all(
        interval[1] - interval[0] <= MAXIMUM_LOG10_INTERVAL_WIDTH for interval in intervals.values()
    )
    result.update(
        {
            "status": "complete/informative" if informative else "unavailable/imprecise-interval",
            "informative": informative,
            "point_log10_density": points,
            "intervals_log10_density": intervals,
        }
    )
    return {**result, "artifact_checksum": canonical_checksum(result)}
