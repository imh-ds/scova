...
def aggregate(
    *,
    ...
    for lane, artifact in artifacts.items():
        ...
        if artifact.get(checksum_field) != canonical_checksum(
            {k: v for k, v in artifact.items() if k != checksum_field}
        ):
            reasons.append(f"{lane} checksum mismatch")
        if (
            artifact.get("program_type") != "qualification"
            or artifact.get("verification_lane") != lane
        ):
        ...
        gate = (
            "all_numerical_agreement_gates_passed"
            if lane == "external"
            else (
                "all_inference_gates_passed"
                if lane == "inference"
                else "all_validation_gates_passed"
            )
        )
    ...
    result = {
        "artifact_type": "scova-cf-qualification-verification-aggregate",
        "schema_version": 1,
        "program_type": "qualification",
        "verification_lane": "aggregate",
        "promotion_decision": "eligible-for-human-promotion-review" if not reasons else "blocked",
        "reasons": reasons,
        "boundary_status": None if boundary is None else boundary.get("status"),
...
def main() -> None:
    ...
    if result["reasons"]:
        raise SystemExit(
            "Qualification verification aggregation blocked:\n- " + "\n- ".join(result["reasons"])
        )
