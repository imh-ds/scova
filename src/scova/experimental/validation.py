...
def target_value(
    ...
    tilt, _ = geometric_tilt_and_gradient(
        propensity, np.array([lam]), active_codes
    )
...
def target_directional_derivative(
    ...
    derivative_tilt = np.einsum(
        "nk,nk->n", gradient[:, 0, :], propensity_direction
    )
    return float(
        np.mean((outcome_regression[:, group] - psi) * derivative_tilt)
        / np.mean(tilt[:, 0])
    )
...
def assignment_eif_inner_product(
    ...
    for row in range(len(propensity)):
        for assigned in range(propensity.shape[1]):
            ...
            influence = (outcome_regression[row, group] - psi) * (
                tilt[row, 0] + q
            ) / eta
...
def population_one_step_estimate(
    ...
    *,
    ...
    if corrected:
        ...
        augmentation += (
            candidate_outcome_regression[:, group] - plug_in
        ) * q_expectation
