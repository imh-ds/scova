...
def _validate_approval(value: Any, role: str) -> None:
    ...
    if set(value) != required or not all(
        isinstance(value[name], str) and value[name] for name in required
    ):
...
def _validate_exclusion(rule: Any) -> None:
    ...
    required = {
        ...
        "runtime_checkable",
        "pre_outcome",
        "independent_of_simulation_outcome",
    }
    if set(rule) != required or not all(
        isinstance(rule[name], bool) for name in required - {"description"}
    ):
    ...
    if not all(
        rule[name]
        for name in ("runtime_checkable", "pre_outcome", "independent_of_simulation_outcome")
    ):
        raise ValueError(
            "Scope exclusions must be runtime-checkable, pre-outcome, and outcome-independent"
        )
    ...
    if any(term in rule["description"].lower() for term in forbidden):
        raise ValueError(
            "Scope exclusions may not encode unobservable DGP truth or observed performance"
        )
...
def validate_record(record: Mapping[str, Any]) -> ScopeDecision:
    ...
    for name in (
        ...
        if not isinstance(record[name], list) or not all(
            isinstance(item, str) and item for item in record[name]
        ):
    ...
    if set(changes) != {
        ...
        "new_matrix_version",
    } or not all(isinstance(value, bool) for value in changes.values()):
...
def validate_registry(values: Mapping[str, Any]) -> dict[str, ScopeDecision]:
    if (
        set(values) != {"schema_version", "records"}
        or values["schema_version"] != DECISION_REGISTRY_SCHEMA_VERSION
    ):
...
def build_qualification_manifest(
    ...
    *,
    ...
    registry: Mapping[str, ScopeDecision] | None = None,
) -> dict[str, Any]:
    ...
    *,
    ...
    for name, value in expected.items():
        if manifest[name] != value:
            raise ValueError(
                f"Qualification decision manifest {name} does not match the frozen context"
            )
    registry = scope_decision_registry() if registry is None else dict(registry)
    ...
    for entry in decisions:
        ...
        if decision.status != "resolved":
            raise ValueError(
                f"Qualification dispatch blocked by unresolved decision {decision.decision_id}"
            )
